...
def build_attempt_history_rows(
    ...
    return build_activity_history_display_rows(
        attempts,
        format_datetime=lambda value: format_user_datetime(value, preferred_timezone),
        get_correct_count=get_correct_count,
    )
...
def render_readiness_card(readiness_display, readiness: Dict[str, Any], passing_score: float, selected_exam: str) -> None:
    st.header("Overall Readiness")
    st.caption("This is a study-planning estimate, not a pass guarantee.")

    # Primary metrics
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Readiness Score", f"{round(readiness_display.readiness_score or 0.0, 2)}%")
    c2.metric("Status", readiness_display.readiness_label)
    c3.metric(
        "Estimate Confidence",
    ...
    r2.metric("Trend", readiness_display.score_trend_indicator)
