...
# Canonical CB-SC-001 production happy path (12 sequential scenes).
HAPPY_PATH_DECISIONS = (
    ...
    (5, "SC001-C05", "opt-sc001-c05-a"),
    (6, "SC001-C06", "opt-sc001-c06-a"),
    (7, "SC001-C07", "opt-sc001-c07-a"),
    (8, "SC001-C08", "opt-sc001-c08-a"),
    (9, "SC001-C09", "opt-sc001-c09-a"),
    (10, "SC001-C10", "opt-sc001-c10-a"),
    (11, "SC001-C11", "opt-sc001-c11-a"),
    (12, "SC001-C12", "opt-sc001-c12-a"),
)
...
def _make_fake_streamlit(session_state: Optional[dict] = None):
    ...
    fake = types.SimpleNamespace(
        ...
        image=lambda *args, **kwargs: rendered.append("image"),
        text_area=lambda *args, **kwargs: "",
        popover=lambda *args, **kwargs: _Ctx(),
        expander=lambda *args, **kwargs: _Ctx(),
        columns=lambda *args, **kwargs: (_Ctx(), _Ctx()) if (args and args[0] != 3) else (_Ctx(), _Ctx(), _Ctx()),
        _rendered=rendered,
        _widget_keys=[],
    )
...
if _DISPOSABLE_BASE_AVAILABLE:
    ...
    from tests.test_scenario_supabase_port_v2 import _scenario_migrations_present
    ...
    class TestScenarioStreamlitV2DisposableIntegration(_PortDisposableSmokeBase):
        ...
        @classmethod
        def _seed_scenario_fixture(cls):
            """Seed the production CB-SC-001 document so Streamlit helpers match published hash."""
            import psycopg2
            from utils.scenario_engine_v2 import ENGINE_VERSION, build_scenario_content_v2

            document = json.loads(CB_SC001_CONTENT_PATH.read_text(encoding="utf-8"))
            content = build_scenario_content_v2(document, source_path=CB_SC001_CONTENT_PATH)
            conn = psycopg2.connect(
                host="127.0.0.1", port=cls.PG_HOST_PORT, user="postgres", dbname="postgres"
            )
