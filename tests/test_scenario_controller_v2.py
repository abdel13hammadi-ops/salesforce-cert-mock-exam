...
class TestEngineV1IsolationAJ(unittest.TestCase):
    def test_aj_v1_controller_module_not_imported_or_modified(self):
        """This module must never import Engine V1's controller.

        The narrow production candidate intentionally excludes Engine V1
        learner pages/controllers. When V1 is present, also assert V1 does
        not import this V2 controller.
        """
        import ast
        ...
        tree = ast.parse(Path(v2_controller.__file__).read_text(encoding="utf-8"))
        ...
        self.assertNotIn("utils.scenario_learner_controller", imported_modules)

        if importlib.util.find_spec("utils.scenario_learner_controller") is None:
            self.skipTest("Engine V1 learner controller excluded from narrow production candidate")

        import utils.scenario_learner_controller as v1_controller

        self.assertNotIn("scenario_controller_v2", v1_controller.__dict__)
