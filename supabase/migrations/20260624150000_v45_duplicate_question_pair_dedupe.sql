-- =============================================================================
-- V45 duplicate-question pair durable idempotency
-- Created : 2026-06-24 15:00:00 UTC
--
-- Purpose
-- -------
--   * Partial unique index on duplicate-question pair identity in audit_findings
--   * list_duplicate_question_pair_keys_v1 RPC for cross-run reads
--   * complete_audit_run_v1 uses ON CONFLICT DO NOTHING for duplicate pair findings
-- =============================================================================

CREATE UNIQUE INDEX IF NOT EXISTS idx_af_duplicate_question_pair_dedupe
...
CREATE OR REPLACE FUNCTION public.list_duplicate_question_pair_keys_v1(
    p_certification_exam_name text,
    p_ruleset_version         text DEFAULT NULL
)
RETURNS TABLE (
    question_version_id_a text,
    question_version_id_b text,
...
AS $$
    SELECT DISTINCT
        af.metadata->>'question_version_id_a',
        af.metadata->>'question_version_id_b',
        af.metadata->>'detection_method',
    ...
    WHERE  af.finding_code IN (
      ...
      AND  COALESCE(af.metadata->>'question_version_id_a', '') <> ''
      AND  COALESCE(af.metadata->>'question_version_id_b', '') <> ''
      AND  COALESCE(af.metadata->>'detection_method', '') <> ''
      AND  (
...
REVOKE ALL ON FUNCTION public.list_duplicate_question_pair_keys_v1(text, text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.list_duplicate_question_pair_keys_v1(text, text) FROM anon;
REVOKE EXECUTE ON FUNCTION public.list_duplicate_question_pair_keys_v1(text, text) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.list_duplicate_question_pair_keys_v1(text, text) TO service_role;

COMMENT ON FUNCTION public.list_duplicate_question_pair_keys_v1(text, text) IS
'Returns canonical duplicate-question pair keys already persisted in audit_findings
for one certification (and optional ruleset_version). Used before repeat scans.';
...
to warning; invalid materiality raises invalid_parameter_value.  Duplicate-question
pair findings use ON CONFLICT DO NOTHING against idx_af_duplicate_question_pair_dedupe
and completion continues when an identical pair is already persisted.  Idempotent on already-completed runs.  Execute permission:
service_role only.  PUBLIC, anon, authenticated revoked.';
