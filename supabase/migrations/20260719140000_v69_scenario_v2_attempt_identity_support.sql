-- =============================================================================
-- SCENARIO_ENGINE_V2 Persistence -- Slice B -- EXECUTABLE MIGRATION
--
-- Task: SIM-PERSIST-V2-03. This is the executable migration derived exactly
-- from the independently reviewed and corrected draft
-- docs/scenario_simulator/SCENARIO_ENGINE_V2_PERSISTENCE_SLICE_A_MIGRATION_DRAFT.sql
-- (Task SIM-PERSIST-V2-02A, corrected under SIM-PERSIST-V2-02C, confirmed
-- READY_FOR_EXECUTABLE_MIGRATION by the independent final review
-- docs/scenario_simulator/SCENARIO_ENGINE_V2_PERSISTENCE_SLICE_A_FINAL_REVIEW.md,
-- Task SIM-PERSIST-V2-02D). Every executable SQL statement below, from BEGIN
-- to COMMIT, is byte-for-byte identical to that reviewed draft -- this header
-- comment block is the only change, updated to reflect that this file is now
-- the real, tracked migration rather than a review-only draft.
--
-- OBJECTIVE
...
-- ambiguity investigation this migration is built on are documented in:
...
-- citation. This migration therefore explicitly DROPs the exact old
-- signature and CREATEs the new one, atomically, in one transaction, so at
-- most one signature ever exists in the catalog, restores every
-- REVOKE/GRANT and the COMMENT (both are attached to the specific function
-- object and are NOT preserved across a DROP), and explicitly asks
-- PostgREST to reload its schema cache rather than relying only on
-- Supabase's own post-migration reload hook.
--
-- SCOPE -- table/column/index/trigger/RLS: NONE.
-- This migration touches exactly one function: public.start_or_resume_
-- scenario_attempt_v1. No table, column, index, trigger, RLS policy, or any
-- other database object is created, dropped, or altered.
-- public.scenario_attempts, public.scenario_decisions, their triggers, their
-- RLS posture, and the other three RPCs (get_scenario_attempt_v1,
-- submit_scenario_decision_v1, abandon_scenario_attempt_v1) are entirely
-- untouched.
--
...
-- draft this migration is copied from)
...
-- file. All four were corrected in the draft this file is copied from
-- verbatim:
...
--   * SA-11-1 (MEDIUM): section 1 captures the six-argument function's
...
--     verified by a postcondition check in section 6.
--   * SA-12-1 (MEDIUM): section 1 verifies a set of material, verbatim body
--     markers (RAISE EXCEPTION message text, SECURITY INVOKER, the ON
--     CONFLICT DO NOTHING shape, return-shape column names) against the
--     installed six-argument function's own pg_get_functiondef() output,
--     aborting before the DROP if the live body has materially diverged
--     from this migration's assumed baseline -- see the v_markers
--     declaration in section 1 for the full rationale on why an exact hash
--     is not hardcoded without live-database access.
-- SA-06-1 (start-idempotency claim) and SA-21-1 (test-plan completeness)
-- were corrected in the contract document
...
-- SCENARIO_ENGINE_V2_PERSISTENCE_SLICE_A_CORRECTION_REPORT.md. Independent
-- confirmation of closure is recorded in
-- SCENARIO_ENGINE_V2_PERSISTENCE_SLICE_A_FINAL_REVIEW.md.
--
-- VALIDATION
-- Applied and validated against a disposable, non-production PostgreSQL
-- database (Docker container, image supabase/postgres:15.1.0.147, never
-- connected to any Supabase project URL/keys) as part of Task
-- SIM-PERSIST-V2-03. Full results:
...
-- ---------------------------------------------------------------------------
-- Note: this block (and the other three DO/function bodies below) use a
-- named dollar-quote tag, rather than the bare "$$" the reviewed draft used,
-- because one of this block's own documentation comments below literally
-- contains the two-character sequence "$$" (describing PL/pgSQL's AS $$...$$
-- function-body delimiters in prose) -- PostgreSQL's dollar-quote scanner
-- matches the first occurrence of the exact tag regardless of whether it
-- falls inside a "--" comment, so a bare "$$" tag here would have the
-- literal "$$" inside that comment prematurely close this block. A named
-- tag (which the literal "$$" in the comment can never match) is the
-- standard, minimal, logic-preserving fix for exactly this class of
-- collision; no DECLARE/BEGIN/END content, comment text, or executable
...
DECLARE
    ...
    -- Empirically discovered during Slice B disposable-database validation
    -- (Task SIM-PERSIST-V2-03): pg_get_functiondef() never emits the literal
    -- text "SECURITY INVOKER" for a SECURITY INVOKER function, because
    -- SECURITY INVOKER is PostgreSQL's default and the deparser
    -- (ruleutils.c) only prints the header clause for the non-default case
    -- (SECURITY DEFINER). A text marker of 'SECURITY INVOKER' can therefore
    -- never match ANY installed SECURITY INVOKER function's
    -- pg_get_functiondef() output, including a perfectly correct,
    -- undrifted baseline -- this is exactly the false-positive-against-a-
    -- genuinely-correct-baseline risk this precondition block's own header
    -- comment (see the file-level rationale above) anticipated could exist
    -- in a hash-free, marker-based design authored without live-database
    -- access, and this task's disposable-database run is precisely what
    -- surfaced it. SECURITY INVOKER is therefore verified below via the
    -- correct mechanism -- the actual pg_proc.prosecdef catalog boolean --
    -- immediately after the marker loop, rather than via a text marker that
    -- cannot ever match.
...
BEGIN
    ...
    -- SECURITY INVOKER verified via the actual catalog boolean (see the
    -- v_markers rationale above for why a text-marker check cannot work
    -- for this specific property).
    SELECT p.prosecdef INTO v_old_prosecdef FROM pg_proc p WHERE p.oid = v_old_oid;
    IF v_old_prosecdef THEN
        RAISE EXCEPTION 'SLICE-A PRECONDITION FAILED: the installed six-argument function is SECURITY DEFINER, expected SECURITY INVOKER. Refusing to proceed without human review -- an unreviewed change may have been applied to this function outside this migration''s own tracked history.';
    END IF;
END
$slice_b_precheck$;
